Here are key terms and thier corresponding definitions to get you started on servicenow University journey:

1. Access control: A security rule defined to restrict the permissions of a user from viewing and interacting with data.

2. Acess control list (ACL): A list in servicenow that defines the permissions for accessing data in tables, including create, read , update, and delete.

3. Application Platform -as-a-service (apaas): A cloud-based service providing tools and infrastructure for developing, deploying, and managing custom applications.

4. Application engine studio(aes): A user friendly tool that enables rapid creation and deployment of applications on the servicenow platform.

5. Application scope: Protects applications by identifiying and restricting access to avaiable arifiacts and data

6. Approcer: a role in servicenow that allows users to view or modify approval records directed to them.

7.Automated test framework (ATF):A set of tools for automating the testing of applications to ensure quality and reduce manual effort.

8. Baseline implement: This refers to a set of installed applicaitons on servicenow instance before any configuration or customization has been done.

9. Base tables: A parent class that is not an extension of another table.

10.Business rule: Configured to run before or after a database action has occured. It can be set to run when a record is displayed, inserted, updated, deleted, or when a table is queried.

11.Capstone project - a final project that typically involves applying the knowledge and skills learned throughout the course.

12. Child class- a table that extends another table.

13.choice List- a type of field that lets the user select from a pred-defined set of choices.

14. CI class Manager: A central location to explore the cmdb class hieracrchy, CI table definitions, and class CI'S.

15. Client - The user interface or front-end of the servicenow platform where users interact with the platform../system

16.Client scripts - scripts created to perform actions such as automatically updating feilds, disabling links, or displaying notices.

17. CMDB - A series of tables and feilds that contain all of the configurations items(Ci's)controlled by your company.

18. Context menus: Provide different levels for a given list view.

19 coalsce fields: Used as a unique key during imports.

20. Core tables- Tables that exist in the base system.

21. Customization: This involves enhancing a portal widget to display lists with complex access controls and adding a table for special billing requirements.

22. Customer actions: Steps that customers can implement and view in the security center, including actions that are available, overdue, due soon, in progress, and complete.

23.  Data pill: A container for data used in subsequent actions within a workflow.

24. Data Policy : A rule that enforces data consistency by setting fileds as manadatory and or read-only. Data policies are applied to all data entered into the platform and execute on the server side but can also run as a UI polociy on the client-side.

25. Dashboards- a feature that enables users to display multiple platform analytics visualizations form different sources and create a story with that can be shared,

26.dependancy view: provides an interactive graphical interface to visualize relationships between configuration items.

27. Discovery: scans the network to inventory devices and applications and updates the cmdb with the results,

28.Dot-walking: Gather information from a series of tables through reference feilds.

29. Encryption at rest: Data encryption that is not enabled by default for customers using the standard now platform. Servicenow offers a suite of encyption options for data at rest.

30.Encryption in transit: Secure access to instances via HTTPS and various methods for data transfer, including APIS.FTPS, AND SCP.

31.feild:  an individual column of data.

32. Feild Attributes: The values are actual data entered into the feild.

33. field label: a user-friendly unqiue term that the system uses to identify the feild.

34. Feild name: A System-friendly unique term that the system uses to identify the field.

35. filters- conditions applied to a table list to isolate a subset of the data.

36. Flow-  a seqeunce of operations executed by the system, such as looking up a reocrd, updating a field values, requestig an approval, or logging a value
...
roups can access a Catalog Item.
